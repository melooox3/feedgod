  ...
  useEffect(() => {
    ...
    if (smartData) {
      try {
        ...
        if (module === 'social' && parsed) {
          // Set platform if valid
          if (parsed.platform) {
            const platform = parsed.platform.toLowerCase() as SocialPlatform
            if (['twitter', 'youtube', 'tiktok'].includes(platform)) {
              setSelectedPlatform(platform)
            }
          }
          // Set username
          if (parsed.username) {
            setTargetInput(parsed.username)
          }
        }
      } catch (e) {
