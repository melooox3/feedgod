...
import { Play, Save, CheckCircle2, XCircle, TrendingUp, TrendingDown, Settings, Clock, Hash, RefreshCw, Plus, Database, Loader2, ExternalLink, Copy, CheckCircle, X, Rocket } from 'lucide-react'
import { FeedConfig, DataSource, Blockchain } from '@/types/feed'
import { fetchCoinGeckoPrice, fetchSourcePrice, generateChartData } from '@/lib/api/price-api'
...
import CustomSourceModal from '@/components/modals/CustomSourceModal'
import {
  DeploymentResult,
  getSolscanLink,
  generateIntegrationCode,
  storeDeployedFeed
} from '@/lib/blockchain/switchboard'
...
import { CHAIN_LOGOS } from '@/lib/constants/chain-logos'
import { CostEstimateDisplay } from '@/components/shared/CostEstimateDisplay'
...
export default function FeedBuilder({ config, onConfigChange }: FeedBuilderProps) {
  ...
  useEffect(() => {
    ...
    if (smartData && localConfig) {
      try {
        const { module, parsed } = JSON.parse(smartData)
        if (module === 'feed' && parsed) {
          const symbol = parsed.base
            ? `${parsed.base}/${parsed.quote || 'USD'}`
            : parsed.query
          if (symbol) {
            handleConfigUpdate({
              symbol,
    ...
    // Only run once when localConfig becomes available
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [localConfig?.id])
