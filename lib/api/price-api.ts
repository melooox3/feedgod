// Real-time price fetching from multiple sources including Switchboard Surge

import { SYMBOL_TO_ID, getCoinGeckoId } from '@/lib/constants/coin-ids'
import { SurgeClient, isSupportedSymbol } from './surge-client'
...
export async function fetchCoinGeckoPrice(symbol: string): Promise<PriceData | null> {
  try {
    const coinId = SYMBOL_TO_ID[symbol] || getCoinGeckoId(symbol)
...
export async function fetchCoinbasePrice(symbol: string): Promise<SourcePrice | null> {
  try {
    // Convert BTC/USD to BTC for Coinbase exchange rates API
    const baseCurrency = symbol.split('/')[0]
...
/**
 * Fetch price from Switchboard Surge (oracle-verified)
 */
export async function fetchSurgePrice(symbol: string): Promise<SourcePrice | null> {
  try {
    ...
    const result = await client.getPrice(symbol)
...
export async function fetchSourcePrice(sourceId: string, symbol: string): Promise<SourcePrice | null> {
  switch (sourceId) {
    ...
    case 'surge':
    case 'switchboard':
      return await fetchSurgePrice(symbol)
    default:
      return null
  }
}
